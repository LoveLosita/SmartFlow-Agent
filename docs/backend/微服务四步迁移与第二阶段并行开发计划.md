...
9. `backend/client` 是跨进程调用客户端层，不属于 gateway 私有目录；gateway 和服务进程都可以 import，但 client 只能依赖 `shared/contracts`、`shared/ports`、rpc/pb 和通用错误适配，不反向依赖 `gateway/api` 或服务私有 `sv/dao/model`。
...
5. 继续收 agent / memory，把聊天编排、主动调度会话复跑和记忆链路独立出去。
6. 最后把 Gin 收口成纯 Gateway。

一句话总结：

> outbox 的服务级基础设施、llm-service、rag-service、user/auth、notification、active-scheduler、schedule、task、course、task-class 都已经完成首轮服务化基线；当前继续把 agent / memory 独立出来，完成聊天编排和记忆链路的服务化；最后把 Gin 收口成真正的 Gateway。
